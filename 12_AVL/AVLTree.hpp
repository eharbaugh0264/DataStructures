...
public:
    ...
    void insert(const T& val); // DONE
    void remove(const T& val); // DONE
    ...
    int getHeight(const AVLNode<T>* node) const; // DONE
    int getBalance(const AVLNode<T>* node) const;

    // Tree rotations
    void rotateRightAt(const T& val);
    void rotateRightRef(AVLNode<T>*& node); // DONE
    void rotateLeftRef(AVLNode<T>*& node);  // DONE
